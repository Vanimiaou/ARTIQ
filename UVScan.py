...
UVfreq=360*MHz#356.5*MHz#348.5*MHz
...
class Main(EnvExperiment):
    ...
    def run(self): 
        ...
        self.zotino0.load()
        delay(1000*ms)
        StartUVFreq=UVfreq-(Npoints-1)*1*0.5*MHz
        #
        #     ********************SET UP SCAN****************
        #
        for p in range(Nloops):
            for m in range(Npoints):
                EndUVfreq=1*(m-0.5*(Npoints-1))*((-1)**p)*MHz+UVfreq
                UVfreqRampSlope=(EndUVfreq-StartUVFreq)/UVfreqRampStep
                for n in range(UVfreqRampStep):
                    self.urukul0_ch3.set(StartUVFreq+UVfreqRampSlope*(n+1))
                    delay(UVfreqTimeStep)
                #UV_pulse_time=m*0.5*us
                scanP=EndUVfreq
                delay(1*ms)
                for j in range(25): #   ********range == # repetitions ********************SET # OF REPETITIONS FOR EACH POINT HERE******
                    ...
                    delay(100*us)
                    #
                    #     ********************OPTICAL PUMPING WITH D1 F=4 -> F'=4****************
                    # 
                    self.zotino0.write_dac(1, OPBiasX) #X coil
                    self.zotino0.write_dac(2, OPBiasY) #Y coil... seems strange to tilt this way.  something to do with uWave polarization?
                    self.zotino0.write_dac(3, OPBiasZ) #Z coil
                    self.zotino0.load()
                    delay(1*ms)
                    self.ttl5.off() #repump ON
                    delay(1*us)
                    ...
                    #     ******************** UWAVE PULSE ****************
                    #
                    self.ttl18.off()#uwave on
                    delay(uwave_pulse_time)
                    self.ttl18.on()#uwave off
                    self.zotino0.write_dac(1, BiasX) #X coil
                    self.zotino0.write_dac(2, BiasY) #Y coil
                    self.zotino0.write_dac(3, BiasZ) #Z coil
                    self.zotino0.write_dac(5, RepumpVVA) #Repump VVA
                    ...
                    #     ********************BLAST F=4****************
                    #
                    for i in range(1): # BLAST PULSES
                        with parallel:
                            with sequential:
                                delay(0.4*us)
                                self.ttl9.on()#trap OFF
                            self.ttl6.off() #blast ON
                        delay(drop_time)# ****DROP ATOM******
                        with parallel:
                            self.ttl6.on() #blast OFF
                            with sequential:
                                delay(0.4*us)
                                self.ttl9.off()#trap ON
                        delay(1*us)"""
                    #
                    #     ********************UV****************
                    #
                    ...
                    delay(100*us)
                    self.zotino0.write_dac(9, StartTweezerVVA) # SUDDEN INCREASE IN TWEEZERS
                    self.zotino0.load()
                    delay(1*ms)
                    #
                    #     ********************DETECT****************
                    #
                    self.ttl5.off() # Repump ON
                    delay(1*us) # Best practice to ensure repump comes on before detection
                    # TURN ON DETECTION LIGHT
                    #if use cooling beam
                    self.ttl4.off()
                    """with parallel:
                        self.ttl6.off()
                        with sequential:
                            ...
                            self.ttl10.off()"""
                    delay(1*us)
                    count_i=self.ttl0.count(self.ttl0.gate_rising(meas_time_probe))# COUNT DETECTION
                    delay(1*ms)
                    #if use cooling beam
                    self.ttl4.on()
                    """with parallel:
                    ...
                    #     ********************OUTPUT DATA****************
                    #
                    print(p,',',m,',',j,',',scanP,',',count_i, ',',smp[0], ',',smp[1]) # PRINT COUNT VALUE
                    delay(50*ms)
